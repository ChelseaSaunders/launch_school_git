...
# (1000, false) == 0; (50000, true) == 25000.  These cover both cases with 
# true and false boolean values.  Other test cases could include instances
# when the input was sometihng other than a positive integer and boolean as
# the first and second arguments, respectively, however for the purposes of
# this exercise I assume we can solve for only the desired input types.

# Data structure.  The inputs are an integer and a boolean respectively, and
# the output is an integer.  The operater (division), needs only to be performed 
# on the integer, and the operator works on the integer, and provides an output
# of an integer--the desired output--so there is no need to alter the data 
# structure.
